...
class G_Template{
  ...
  function __construct(){
    ...
    if ($c->getDefaultFwModule('translate')){
      $tr = new G_Translate();
  ...
  public function process(){
    ...
    if (!$this->getJson()){
      ...
      foreach ($css as $css_item){
        ...
        $str_css .= '<link rel="stylesheet" media="screen" type="text/css" href="/'.$this->getFolderUrl().'css/'.$css_item.'.css" ';
      ...
      foreach ($js as $js_item){
        $str_js .= '<script type="text/javascript" src="/'.$this->getFolderUrl().'js/'.$js_item.'.js"></script>'."\n";
      }
    ...
    // Añado traducciones
    if ($c->getDefaultFwModule('translate') && $this->getTranslator()->getPage()!=''){
