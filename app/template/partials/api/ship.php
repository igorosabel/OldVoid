...
<?php $s = $values['ship']; ?>
{
  ...
  "hull_type_name": "<?php echo urlencode($values['ship_service']->getHullTypeName($s->get('hull_id_type'))) ?>",
  ...
  "shield_type_name": "<?php echo urlencode($values['ship_service']->getShieldTypeName($s->get('shield_id_type'))) ?>",
  ...
  "engine_type_name": "<?php echo urlencode($values['ship_service']->getEngineTypeName($s->get('engine_id_type'))) ?>",
  ...
  "generator_type_name": "<?php echo urlencode($values['ship_service']->getGeneratorTypeName($s->get('generator_id_type'))) ?>",
...
<?php foreach ($s->getGuns() as $i => $gun): ?>
    {
      ...
      "gun_type_name": "<?php echo urlencode($values['ship_service']->getGunTypeName($gun->get('id_type'))) ?>",
  ...
  <?php foreach ($s->getModulesSmall() as $i => $module): ?>
    {
      ...
      "module_type_name": "<?php echo urlencode($values['ship_service']->getModuleTypeName($module->get('id_type'))) ?>",
      ...
      <?php foreach ($module_enables as $j=>$enable): ?>
          {
            "id": "<?php echo $enable ?>",
            "enable_name": "<?php echo urlencode($values['ship_service']->getModuleEnableName($enable)) ?>"
    ...
    "storage": <?php if ($module->get('storage')==''): ?>false<?php else: ?>
      ...
      <?php foreach ($storage as $resource): ?>
        ...
        "resource_name": "<?php echo urlencode($values['general_service']->getResourceName($resource['resource_id'])) ?>",
